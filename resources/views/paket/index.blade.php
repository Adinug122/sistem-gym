<x-app-layout>
 ...
 <div class="mb-6 flex justify-between items-center  gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <h1 class="text-2xl font-bold text-gray-800">
                Dashboard <span class="text-gym-600">Paket Memberhsip</span>
            </h1>

            <a href="{{ route('admin.paket.create') }}" class="inline-flex items-center justify-center px-4 py-2 bg-gym-600 text-white rounded-lg hover:bg-gym-500 transition-colors shadow-lg shadow-gym-600/20">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <thead class="bg-gray-50 text-gray-500 border-b border-gray-100">
                    <tr>
                        <th class="px-6 py-4 text-xs font-bold uppercase tracking-wider">Nama paket</th>
                        ...
                        <th class="px-6 py-4 text-xs font-bold uppercase tracking-wider">Benefits</th>
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                        <span >{{ $item->durasi_angka }} </span>
                        @if ($item->durasi_satuan == 'day')
                        Hari                            
                        @elseif ($item->durasi_satuan == 'month')
                        Bulan
                        @elseif ($item->durasi_satuan == 'year')
                        Tahun                            
                        @endif
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                            @if ($item->benefits)
                                @foreach ($item->benefits as $benefits)
                                    {{ $benefits }}
                                @endforeach
                            @endif
                        </td>
                 ...
                 <a href="{{ route('admin.paket.edit',$item->id) }}" class="text-gym-600 hover:text-gym-900 transition-colors" title="Edit Data">
    <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <path d="M12.5 15.8L22 6.2L17.8 2L8.3 11.5L8 16L12.5 15.8Z" 
              stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                ...
                                <button @click="isOpen = true; actionUrl='{{ route('admin.paket.destroy',$item->id) }}'" class="z-20 "> 
